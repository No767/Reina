...
from reina_genshin_wish_sim import ReinaGWSCacheUtils, ReinaGWSUtils, WSUser, WSUserInv
from reina_ui import GWSDeleteOneUserInvItemModal, GWSPurgeInvView
...
POSTGRES_WS_DATABASE = os.getenv("Postgres_Reina_Database")
...
MODELS = ["reina_genshin_wish_sim.models"]

gwsUtils = ReinaGWSUtils(uri=CONNECTION_URI, models=MODELS)
cacheUtils = ReinaGWSCacheUtils(
    uri=CONNECTION_URI, models=MODELS, redis_host=REDIS_HOST, redis_port=REDIS_PORT
)


class GWS(commands.Cog):
    """Reina's Genshin Wish Simulator (GWS)"""

    def __init__(self, bot):
        self.bot = bot

    gws = SlashCommandGroup("gws", "Commands for Reina's Genshin Wish Sim")
    ...
    async def gwsWishOne(self, ctx: discord.ApplicationContext):
        ...
        async with ReinaCM(uri=CONNECTION_URI, models=MODELS):
