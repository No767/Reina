...
wsUserInvUtils = ReinaWSUserInvUtils()
wsUserUtils = ReinaWSUsersUtils()
        ...
        try:
            if len(mainRes) == 0:
                ...
                for items in mainRes:
                    ...
                    embed.add_field(
                        name="Pulls", value=dict(items)["pulls"], inline=True
                    )
                    embed.add_field(
                        name="Date Joined",
            ...
            embedError = discord.Embed()
            embedError.description = "Sorry, but it seems like you don't have a profile yet. Please make your first wish in order to create a profile."
            await ctx.respond(embed=embedError)
